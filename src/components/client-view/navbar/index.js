...
function CreateMenus({ activeLink, getMenuItems, setActiveLink }) {
  return getMenuItems.map((item) => (
    ...
    ${
      activeLink === item.id
        ? "text-color animation-active shadow-color"
        : "text-[#000] font-bold hover-color"
    }
    `}
    >
      {item.label}
      ...
      >
        <nav className="max-w-screen-xl px-6 sm:px-8 lg:px-16 mx-auto grid grid-flow-col py-3 sm:py-4">
          <div className="col-start-1 col-end-2 flex items-center">
            <div className="cursor-pointer flex gap-2 font-bold items-center text-[20px] text-color">
              <div className="w-[40px] h-[40px] flex justify-center items-center p-3 rounded-[8px] boder-color color">
