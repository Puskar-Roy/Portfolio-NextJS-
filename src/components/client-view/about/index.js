...
import aboutMeImage from "../../../assets/about-image2.gif";
  ...
  return (
    ...
    >
      <div className="w-full flex">
        <AnimationWrapper className="rounded-lg w-full grid-flow-row grid grid-cols-1 sm:grid-cols-3 py-9 divide-y-2 sm:divide-y-0 sm:divide-x-2 boder-color-divided bg-ehite-500 z-10">
            ...
            >
              <div className="flex m-0 w-40 sm:w-auto">
                <div className="flex flex-col">
                  <p className="text-[50px] text-color font-bold">
                    {infoItem.value}+
                  </p>
        ...
        <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
          <h1 className="leading-[70px] mb-4 text-3xl lg:text-4xl xl:text-5xl font-medium">
            {headingText.split(" ").map((item, index) => (
              <span className={`${index === 6 ? "text-color" : "text-[#000]"}`}>
                {item}{" "}
              </span>
            ))}
          </h1>
            ...
            {data?.skills &&
              data.skills.split(",").map((skill) => (
                <motion.div
                  ...
                  key={skill} // It's a good practice to provide a unique key when mapping over an array
                >
                  <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[160px] px-6 border-[2px] border-color bg-[#fff] text-[#000] font-semibold rounded-lg text-xl tracking-widest hover-color transition-all outline-none">
                    {skill}
                  </button>
                </motion.div>
              ))}
